...
目前公開測試版本為 `0.1.2 Beta`。請到 [GitHub Releases](https://github.com/swallowkog/input-method-workflow-settings/releases) 下載 `Input Method Agent Installer.pkg`。

Beta 測試重點與已知限制請見 [0.1.2 Beta 測試指南](docs/BETA_TESTING.md)。
...
第一次啟動會自動打開設定與權限引導；之後需要修改規則時，點選單列的鍵盤圖示，再選「開啟設定」。
